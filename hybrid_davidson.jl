...
function davidson(
    ...
    while nevf < l_buffer
        ...
        deg_groups = degeneracy_detector(Σ_sorted; tol = 1e-2)

        conv_indices = Int[]  # global indices to lock

        for group in deg_groups
            # group entries refer to sorted positions
            # retrieve residuals of this group
            res_ok = all(norms_sorted[i] < thresh for i in group)

            if res_ok
                # lock entire cluster
                for gi in group
                    push!(Eigenvalues, Σ_sorted[gi])
                    Ritz_vecs = hcat(Ritz_vecs, X_sorted[:, gi])
                    V_lock = hcat(V_lock, X_sorted[:, gi])

                    original_index = sorted_indices[gi]
                    push!(conv_indices, original_index)
                    nevf += 1
                end
            end
        end

        # Exit if enough eigenpairs converged
        if nevf >= lc
            println("Converged all required eigenvalues (cluster-based).")
            return (Eigenvalues, Ritz_vecs)
        end
